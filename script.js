...
// 全域變數存儲課程資料（由Google Apps Script自動推送更新）
// Google Apps Script會定時從Google Sheets讀取資料並直接更新這個陣列到GitHub
let dynamicCourseData = [];

// 生成排序後的課程表格HTML（從Google Apps Script自動推送的資料）
function generateSortedCourseTable() {
    ...
    const processedCourses = dynamicCourseData.map(course => {
        ...
        if (date1 && date2) {
            if (date1 <= today && date2 >= today) {
                ...
                // 兩天都過了 - 不顯示，除非是最近的3堂課之一
                shouldShow = false;
            }
        } else if (date1) {
            if (date1 >= today) {
                status = date1.toDateString() === today.toDateString() ? '進行中' : '即將到來';
    ...
    const sortedCourses = processedCourses
        ...
        .sort((a, b) => {
            ...
            if (b.status === '進行中' && a.status !== '進行中') return 1;
    ...
    const tableRows = sortedCourses.map(course => {
        const courseId = getCourseIdFromName(course['課程名稱']);
        const scheduleText = formatScheduleText(course['上課日期1'], course['上課日期2']);
        
        return `
        <tr>
            <td><a href="javascript:void(0)" onclick="showCourseDetail('${courseId}'); return false;" class="course-link">${course['課程名稱']}</a></td>
            <td>${scheduleText}</td>
            <td>${course['上課時間'] || ''}</td>
...
// 這個函數已被移除，因為現在直接透過Google Apps Script推送資料到GitHub
// 課程資料會自動更新到 dynamicCourseData 陣列中
